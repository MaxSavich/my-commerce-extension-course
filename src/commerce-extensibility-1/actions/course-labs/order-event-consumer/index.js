...
function normalizeScopes (raw) {
  ...
  if (typeof raw === 'string') {
    const s = raw.trim();
    if (s.startsWith('[')) {
      try { arr = JSON.parse(s); } catch (_) { arr = s; }
    }
...
async function getImsAccessToken (params) {
  ...
  const scope = normalizeScopes(params.IMS_OAUTH_S2S_SCOPES) || DEFAULT_SCOPES;
...
async function main (params) {
  ...
  try {
    ...
    if (!params.IMS_OAUTH_S2S_CLIENT_SECRET) missing.push('IMS_OAUTH_S2S_CLIENT_SECRET');
    if (missing.length) {
