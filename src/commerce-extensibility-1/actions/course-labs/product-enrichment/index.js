  ...
  try {
    ...
    if (!response.ok) {
      logger.error(JSON.stringify({
        action: 'product-enrichment', message: 'Commerce API error',
      ...
      if (response.status === 404) {
        body.hint =
            'For ACCS (api.commerce.adobe.com) paths are /V1/products/{sku}, not /rest/.... Confirm SKU exists in this tenant.';
      }
    ...
    logger.info(JSON.stringify({
      action: 'product-enrichment', message: 'Product enriched',
    ...
    logger.error(JSON.stringify({
      action: 'product-enrichment', message: 'Action failed',
